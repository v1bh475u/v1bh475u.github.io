...
A bit about my chip: Intel chips have evolved to have 2 different types of cores: P-cores and E-cores. P-cores are performance cores each equipped with its own private L1 and L2 caches and optimized for performance. These are mostly responsible for high energy consumption. E-cores, on the other hand, are efficiency-driven. They share L2 cache. For my purpose, I was interested in conducting the experiment on a P-core as it follows the same topology as used in textbooks- L1, L2 private and L3 as shared cache.
...
Before I start measuring anything, I had to do a **LOT** of isolation and cleaning to keep the noise down.

### The Compiler

Modern compilers are smart enough to detect and optimize memory accesses. So, I cannot use sequential memory access or else it might get vectorized or become something else entirely. To prevent this, I used random pointer chase. I allocate the working set and then starting from a random address, the next access address is determined by the value at the current address. I also used inline asm block so that compiler won't mark the loop as dead code. I also padded the node size to be equal to be 1 cache line so that 1 node access is touching exactly 1 cache line.
...
It can pre-empt my program or migrate it to some other core, so I pinned my program to logical core `7`, offlined its SMT sibling logical CPU `6`, forced the `performance` governor, and disabled turbo. CPU governor decides the CPU frequency. In other modes, the frequency can change mid-execution. `performance` governor keeps the frequency at its maximum non-turbo frequency. What is turbo frequency? Another Intel quirk. It allows the frequency to rise temporarily above the base level when there is thermal and power headroom. So, obviously, I need to disable turbo boost as well.
...
```
`rdtsc` reads the CPU's timestamp counter and `rdtscp` does the same but only after all prior instructions have completed. `lfence` is an execution barrier that makes sure that measured loop does not move across timing boundary.

Even after this, there are extremely clever prefetchers that would render any logical access pattern into a more sequential one. To prevent that, I used pure random order generation for the pointer chase.
...
After ensuring all the above precautions, I am left with the following details:
| Knob                  |                                Value |
| --------------------- | -----------------------------------: |
...
The L1 and L2 regions are pretty clear. The sequential chain does not show the staircase rise because it has extremely predictable access pattern that the every next access is prefetched into L1d directly, thus giving us the L1 latency for all the working sets.

The issue here is regarding L3 cache. I ran the experiment multiple times and still the same result! Now, as per my specs, L2 region ends at `1.25 MiB` and L3 begins. It ends at `12 MiB`. Instead, I got a cliff! The graph jumps to around `275 cycles/access` starting from `4 MiB` itself!

This made me wonder what was wrong. I know that L3 is shared and there will be noise due to live OS but this was just too much.
...
From the graph, it is clear that explicit huge pages and THP ones have slightly lower latencies but still contain the sharp rise as the normal page graph.
...
Maybe my benchmark was getting some unfortunate placement causing many of the addresses to be in the same set or slices and after averaging those unlucky ones, I might be getting the cliff.
...
Interesting... The cliff is because I am getting a lot of L3 misses where I shouldn't. At `4 MiB` itself, the miss rate is more than 50%! Also, strangely enough, the miss rate drops a little from `8 MiB`. I believe the miss rate should ideally stay closer to zero or something up to `8 MiB` and then rise considering the OS noise.
...
In the original experiment, I was using fresh allocation per working-set size. So, I was not just increasing the working-set size, I was also potentially changing physical bakcing per size. These fresh allocations could be introducing some bad mapping for some sizes making them look bad. So, to test whether these fresh allocations truly affected my previous results or not, I decided to conduct a test.
...
The result seemed promising! Maybe the allocation mattered here. The fresh-allocation runs showed rapid rise in L3 misses while the prefixes of one fixed backing stayed low as ideally the misses should.
...
Now, I have no idea why did this work! But I have speculations. From the previous sections, I know that backing strategy did not give me the answer. The remaining suspects are post-boot memory state and prefetchers.
...
Either this or it was prefetchers messing up my readings due to reading up accidental patterns and prefetching the wrong lines. Seems unlikely but can be possible.

Now, this seems like a rabbit hole to dive into. But for now, I am going to end here. Wait for the next blog.
<!-- TODO: improve the ending -->
## References
